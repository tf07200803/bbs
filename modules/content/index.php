...
class index {
    ...
    public function getorder(){
        ...
        $link_db = pc_base::load_model(list_model);
        $result['data'] = $link_db->listinfo('','id desc',$_GET['page'],30);
        $result['pages']=$link_db->pages;



        alert::message(1,'讀取成功',$result);
    ...
    public function getlist(){
        $link_db = pc_base::load_model(list_model);
        $result = $link_db->get_one(array('id'=>$_GET['id']));
        alert::message(1,'讀取成功',$result);
	}

    public function getshop_one(){
        $link_db = pc_base::load_model(list_model);
        $title=$_POST['title'];
        $intro=$_POST['intro'];
        $imgsrc=$_POST['imgsrc'];
        ...
        $list = $link_db->select(array('eqno'=>$eqno));
        if($list){
            echo $eqno.'有了\n\n';
        }else{
            echo $eqno.'沒有\n';
	...
	public function getshop(){
	   // echo 'yayayaya';
        ...
        foreach($arr as $field=>$value) {
            ...
            $list = $link_db->select(array('eqno'=>$value->eqno));
            //var_dump($list) ;
			if($list){
				echo $value->eqno.'有了\n\n';
			}else{
